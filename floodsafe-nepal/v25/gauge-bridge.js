(()=>{'use strict';
if(window.__fsGaugeBridgeV2)return;window.__fsGaugeBridgeV2=true;
...
function norm(s){return String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\b(river|khola|nadi|stream|station|gauge|at|near|bridge|highway|bazar|bazaar)\b/g,' ').replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ').trim()}
...
function candidate(o,la,lo){const a=lat(o),b=lon(o);if(a===null||b===null||!fresh(o))return null;const s=stage(o);return{o,name:name(o),lat:a,lon:b,distanceKm:km(la,lo,a,b),time:stamp(o),statusLabel:statusLabel(s.status),...s}}
function nearby(la,lo,maxKm=80,limit=12){const stations=window.FloodSafe?.state?.stations||[];return stations.map(o=>candidate(o,la,lo)).filter(Boolean).filter(x=>x.distanceKm<=maxKm).sort((a,b)=>a.distanceKm-b.distanceKm).slice(0,limit)}
function forWaterway(w,la,lo){const wn=String(w?.name_en||w?.name||w?.name_ne||'');const all=nearby(la,lo,180,80);const direct=all.filter(x=>nameMatch(wn,x.name)&&x.distanceKm<=140).sort((a,b)=>a.distanceKm-b.distanceKm)[0]||null;if(direct)return{...direct,sameRiver:true,reference:false};const ref=all.find(x=>x.distanceKm<=80)||null;return ref?{...ref,sameRiver:false,reference:true}:null}
window.FloodSafeGauge={forWaterway,nearby,nameMatch,statusLabel};
})();