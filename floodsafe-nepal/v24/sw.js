const CACHE='floodsafe-nepal-v24-shell-5';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon.svg','./v23-hotfix.js','./v24-safety-guard.js','./v24-upgrade.js'];
...
self.addEventListener('fetch',e=>{
  ...
  if(/\/v2[34]\/v23-upgrade\.js$/.test(u.pathname)){
    ...
    ]).then(async([base,hot,guard])=>{
      if(!base.ok)throw Error('base '+base.status);
      const a=await base.text(),b=hot.ok?await hot.text():'',c=guard.ok?await guard.text():'';
      return new Response(a+'\n;'+b+'\n;'+c,{status:200,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}});
