...
function show(){open=true;renderButton();setTimeout(()=>{try{window.FloodSafeMap?.map?.resize?.()}catch{}window.dispatchEvent?.(new CustomEvent('fsmapvisibility',{detail:{open:true}}));$('map')?.scrollIntoView?.({behavior:'smooth',block:'start'})},80)}
