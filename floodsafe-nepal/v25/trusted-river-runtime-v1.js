...
const MAX=20*60*1000,FUTURE=5*60*1000,POLL=1000;
...
function enforce(source='official mirror'){const S=window.FloodSafe?.state;if(!S)return;S.stations=trusted.slice();S.lastPoll=last||Date.now();S.feedSource=`${source} • ${trusted.length} current ≤20m`;const detail={count:trusted.length,lastFetch:last,source,generatedAt:lastGenerated};window.dispatchEvent(new CustomEvent('fstrustedriverupdate',{detail}));window.dispatchEvent(new CustomEvent('fsriverupdate',{detail}))}
async function poll(){if(busy||document.hidden||!navigator.onLine)return;busy=true;try{const j=await getJson(SNAP+'?official_mirror='+Date.now()),all=Array.isArray(j?.rivers)?j.rivers:[],next=all.filter(valid),generated=String(j?.generated_at||'');trusted=next;last=Date.now();lastGenerated=generated;enforce('DHM/BIPAD official mirror')}catch(e){console.warn('FloodSafe official river mirror poll failed',e);enforce(trusted.length?'last-good official mirror':'official mirror unavailable')}finally{busy=false}}
