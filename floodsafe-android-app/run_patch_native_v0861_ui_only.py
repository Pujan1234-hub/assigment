...
if cs<0 or ce<0: raise SystemExit('v0861 ui wrapper StationDot class missing')
block=m[cs:ce]
if 'V0861_MAP_STATION_DETAIL_FIELDS' not in block:
    dm=re.search(r'double\s+([^;]+);',block)
    if not dm: raise SystemExit('v0861 ui wrapper StationDot double fields missing')
    names=dm.group(1);add=[]
...
if 'V0861_READ_STATION_DETAIL' not in m:
    ...
    if anchor not in m: raise SystemExit('v0861 ui readStation level anchor missing')
...
tmp=root/'_patch_native_v0861_ui_runtime.py'
...
if 'v0859RefreshStaticText(root);' in a: raise SystemExit('v0861 ui stale language helper remains')
a_path.write_text(a,encoding='utf-8')
print('FloodSafe v0.8.61 UI/map/language wrapper PASS (data pipeline intentionally left for v0.8.62)')
