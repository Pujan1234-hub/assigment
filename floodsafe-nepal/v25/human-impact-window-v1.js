...
function sameEvent(a,b){const x=eventKey(a),y=eventKey(b);if(!x||!y)return false;if(x===y)return true;const compact=s=>s.normalize('NFKC').replace(/[^a-z0-9\u0900-\u097f]+/g,'');const cx=compact(x),cy=compact(y);if(cx===cy)return true;const bhote=s=>(s.includes('bhote')&&s.includes('koshi'))||(s.includes('भोटे')&&s.includes('कोशी'));return bhote(x)&&bhote(y)}
...
async function check(){
  ...
  let j=null;if(live&&metrics(live))j=live;else if(fb&&metrics(fb))j=fb;
  if(!j){setVisible(false,'no-human-impact');return}
  let start=startTime(j);if(fb&&sameEvent(j,fb)&&t(fb.event_start_iso))start=t(fb.event_start_iso);
