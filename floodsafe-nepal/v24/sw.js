const CACHE='floodsafe-nepal-v24-shell-8';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon.svg','./v23-hotfix.js','./v23-lang-police.js','./v23-river-map-v3.js','./v24-river-display.js','./v24-safety-guard.js','./v24-upgrade.js'];
...
self.addEventListener('fetch',e=>{
  ...
  if(/\/v2[34]\/v23-upgrade\.js$/.test(u.pathname)){
    e.respondWith(Promise.all([
      ...
      fetch('./v24-river-display.js?v=1',{cache:'no-store'})
    ]).then(async([base,hot,guard,live,riverDisplay])=>{
      if(!base.ok)throw Error('base '+base.status);
      const a=await base.text(),b=hot.ok?await hot.text():'',c=guard.ok?await guard.text():'',d=live.ok?await live.text():'',e=riverDisplay.ok?await riverDisplay.text():'';
      return new Response(a+'\n;'+b+'\n;'+c+'\n;'+d+'\n;'+e,{status:200,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}});
