...
# 2) publish every actual waterway intersecting the current viewport (no arbitrary top-N);
# 3) include precise camera position in the refresh key so panning inside one tile refreshes;
# 4) keep local OSM tile geometry exact: no 77-district re-clipping at local zoom;
# 5) keep national map clean and same-river BIPAD/DHM status semantics.
        ...
        io.execute(()->{
            try{
                ...
                if(zoom<7.0){
                    ...
                    // Local tile geometry is already Nepal-sourced OSM truth. Do not re-clip it
                    // against 77 district rings; that was deleting/fragmenting urban khola.
...
# Remove the broken outside-mask installation. It caused the black diagonal band in field screenshots.
...
m,nmask=mask_block.subn('\n            // v0.8.22: no outside fill mask; local river tiles are Nepal-sourced.\n',m,count=1)
...
# National map: only severe gauges; normal dots appear locally.
...
for marker in [
    'riverIntersectsBox(r,fw,fs,fe,fn)','Math.round(cla*200.0)','chosen.addAll(candidates);',
    'readRiverTile(x,y,null)','lineColor("#18e6ff"), lineWidth(3.2f)','lineColor("#003f55"), lineWidth(7.0f)',
    'local?stationGeo(snapshot, "normal"):emptyFeatureCollection()','sameRiverGaugeFor',
...
if 'style.addLayer(new FillLayer("fs-nepal-outside-mask-layer"' in m2: raise SystemExit('broken outside mask layer still installed')
if 'zoom>=10.0?candidates.size()' in m2: raise SystemExit('whole dense tile publish logic remained')
print('FloodSafe v0.8.22 exact viewport rivers + black-mask removal PASS')
