...
function hideNow(){const el=$('humanStatus');if(el)el.hidden=true;window.__fsHumanStatus={active:false,incidents:0,totals:{dead:0,injured:0,missing:0,rescued:0},checkedAt:new Date().toISOString(),windowDays:10}}
function render(active,incidents,losses,signal){const el=$('humanStatus');if(!el)return;if(!active){hideNow();return}el.hidden=false;const ids=new Set(incidents.map(id).filter(Boolean));const totals={dead:0,injured:0,missing:0,rescued:0},seen={dead:false,injured:false,missing:false,rescued:false};for(const l of losses){const rid=rel(l);if(rid&&ids.size&&!ids.has(rid))continue;if(!rid&&!recent(l))continue;const k=lossKind(l),n=count(l);if(!k||n===null)continue;totals[k]+=n;seen[k]=true}
...
async function poll(){try{const sig=await newsSignal();if(!sig.length){hideNow();return}const [incR,lossR]=await Promise.allSettled([get(API+'incident/?limit=500&ordering=-incidentOn'),get(API+'loss-people/?limit=1000&ordering=-createdOn')]);const incidents=incR.status==='fulfilled'?rows(incR.value).filter(x=>recent(x)&&isDisaster(x)):[];const losses=lossR.status==='fulfilled'?rows(lossR.value):[];render(true,incidents,losses,sig)}catch{hideNow()}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{hideNow();poll();setInterval(poll,POLL)},{once:true});else{hideNow();poll();setInterval(poll,POLL)}})();
