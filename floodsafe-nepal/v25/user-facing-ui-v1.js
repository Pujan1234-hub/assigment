...
const stations=window.FloodSafe?.state?.allRiverStations||[],available=stations.filter(x=>x._lastKnownObservation).length;
let list=$('sourceReadingList');if(!list&&$('nationalRivers')){list=document.createElement('div');list.id='sourceReadingList';$('nationalRivers').appendChild(list)}
const escape=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
if(list){const html='<h4>'+tr('पछिल्लो उपलब्ध मापन — स्रोत समय सहित','Latest available readings — source time shown')+'</h4>'+stations.map(x=>{const o=x._lastKnownObservation;return '<div class="newsItem"><b>'+escape(window.FloodSafeRiverRealtime?.name(x)||x.title||'Station')+'</b><div>'+ (o?escape(o.level)+' m • '+escape(new Date(o.time).toLocaleString(lang()==='en'?'en-GB':'ne-NP',{timeZone:'Asia/Kathmandu'}))+' NPT':tr('मापन उपलब्ध छैन','No reading available'))+'</div></div>'}).join('');if(list.innerHTML!==html)list.innerHTML=html}
