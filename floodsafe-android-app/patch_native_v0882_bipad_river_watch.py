...
if start<0:start=a.find('List<RiverStation> loadTrustedRiverStationsV862')
...
pos2=pos+len(needle)
loader=loader[:pos2]+'\n                    if(path.startsWith("river/?"))v882RiverWatchKeys.add(key);else if(path.startsWith("river-trimed/?"))v882TrimedKeys.add(key); // V0882_ACTIVE_BIPAD_MEMBERSHIP\n'+loader[pos2:]
...
pos2=pos+len(needle)
# IMPORTANT: trailing newline keeps the following JSONObject row declaration live Java code.
...
a=a.replace('"🌊 आधिकारिक catalog "+v849CatalogCount+" • map मा "+mapRiverTotal+" • LIVE "+v877FreshObservationCount+" • latest reading "+v877LatestObservationCount+" • reading नभएको "+v877NoObservationCount','"🌊 BIPAD River Watch "+v849CatalogCount+" • map मा "+mapRiverTotal+" • recent reading "+v877FreshObservationCount+" • latest reading "+v877LatestObservationCount',1)
a=a.replace('"🌊 Official catalog "+v849CatalogCount+" • mapped "+mapRiverTotal+" • LIVE "+v877FreshObservationCount+" • latest reading "+v877LatestObservationCount+" • no reading "+v877NoObservationCount','"🌊 BIPAD River Watch "+v849CatalogCount+" • mapped "+mapRiverTotal+" • recent reading "+v877FreshObservationCount+" • latest reading "+v877LatestObservationCount',1)
if 'V0882_RIVER_WATCH_HEADER' not in a:
    marker='private void updateMapHintCounts()';p=a.find(marker)
