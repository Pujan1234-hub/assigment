(()=>{'use strict';
if(window.__fsRiverLineStyleV8)return;window.__fsRiverLineStyleV8=true;
const COLOR=['match',['get','live_status'],'danger','#ff1515','warning','#ff7a00','watch','#ffd166','normal','#22b8ff','#557b8b'];
const OPACITY=['match',['get','live_status'],'unknown',0.55,0.99];
const ALERT_FILTER=['in',['get','live_status'],['literal',['warning','danger']]];
const LIVE_FILTER=['in',['get','live_status'],['literal',['normal','watch','warning','danger']]];
function addLayers(map,source){if(!map?.getSource(source))return;
  try{
...
const FLOW_PATTERNS=[[0.2,2.4],[0.55,2.0],[0.95,1.6],[1.35,1.2],[0.95,1.6],[0.55,2.0]];
let frame=0,phase=false,anim=null;
function animate(){const map=window.FloodSafeMap?.map;if(!map)return;frame=(frame+1)%FLOW_PATTERNS.length;phase=!phase;try{
  if(map.getLayer('hydro-complete-live-flow')){
    map.setPaintProperty('hydro-complete-live-flow','line-dasharray',FLOW_PATTERNS[frame]);
    map.setPaintProperty('hydro-complete-live-flow','line-opacity',phase?0.9:0.58);
  }
  if(map.getLayer('hydro-complete-flood-glow')){
    map.setPaintProperty('hydro-complete-flood-glow','line-opacity',phase?0.82:0.38);
    map.setPaintProperty('hydro-complete-flood-glow','line-width',['interpolate',['linear'],['zoom'],5,phase?14:9,10,phase?24:17,16,phase?38:28]);
  }
  if(map.getLayer('hydro-complete-flood-pulse')){
    map.setPaintProperty('hydro-complete-flood-pulse','line-dasharray',phase?[0.35,1.0]:[1.15,0.55]);
    map.setPaintProperty('hydro-complete-flood-pulse','line-opacity',phase?1:0.72);
  }
}catch{}}
...
window.FloodSafeRiverStyle={apply,COLOR,get dangerColor(){return'#ff1515'},get warningColor(){return'#ff7a00'},get normalColor(){return'#22b8ff'},get unknownColor(){return'#557b8b'}};
})();