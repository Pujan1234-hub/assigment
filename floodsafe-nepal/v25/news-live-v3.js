...
function render(){const el=out();if(!el)return;const now=Date.now(),seen=new Set();const list=[...ronbCache,...mediaCache].filter(x=>x.title&&ts(x.time)>0&&now-ts(x.time)>=-300000&&now-ts(x.time)<=FRESH).filter(x=>{const k=x.title.toLowerCase().replace(/\W+/g,' ').trim();if(!k||seen.has(k))return false;seen.add(k);return true}).sort((a,b)=>ts(b.time)-ts(a.time)).slice(0,30);el.innerHTML='';if(!list.length){el.innerHTML='<div class="empty"><strong>अहिले पछिल्लो ६० मिनेटमा नयाँ verified national news छैन।</strong><br>पुरानो समाचार देखाइएको छैन।</div>';return}for(const x of list){const a=document.createElement('article');a.className='newsItem'+(/ronb/i.test(x.source)?' ronbNews':'');a.innerHTML=`<h4>${esc(x.title)}</h4><div class="meta">${esc(x.source)} • ${esc(age(x.time))} • ${esc(npt(x.time))} <b class="liveDot">NEW</b></div>${x.url?`<a href="${esc(x.url)}" target="_blank" rel="noopener noreferrer">समाचार खोल्नुहोस् ↗</a>`:''}`;el.appendChild(a)}}
...
function boot(){const m=document.querySelector('#bulletin .head .muted');if(m)m.textContent='RONB + verified राष्ट्रिय मिडिया • पछिल्लो ६० मिनेटका नयाँ पोस्ट मात्र';syncRonb(true);syncMedia(true);setInterval(()=>syncRonb(false),10000);setInterval(()=>syncMedia(false),60000);setInterval(render,15000);document.addEventListener('visibilitychange',()=>{if(!document.hidden){syncRonb(true);syncMedia(true)}});window.addEventListener('online',()=>{syncRonb(true);syncMedia(true)})}
