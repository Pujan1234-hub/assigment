...
function renderButton(){const b=$('fsOpenMapBtn'),card=$('map');if(!b||!card)return;card.classList.toggle('fsMapClosed',!open);card.classList.toggle('fsMapOpen',open);const v=open?tr('✕ नदी नक्सा बन्द गर्नुहोस्','✕ Close River Map'):tr('🗺️ नदी नक्सा खोल्नुहोस्','🗺️ Open River Map');if(b.textContent!==v)b.textContent=v}
...
function bindNav(){for(const a of document.querySelectorAll('.bottom a'))if(a.getAttribute('href')==='#map')a.addEventListener('click',()=>{show()},true)}
