...
# v0.8.35 currently needs this compatibility adjustment before application.
v835 = root / 'patch_native_v0835_fast_overview_rain_taps.py'
text = v835.read_text(encoding='utf-8')
text = text.replace("    'readRiverTile(x,y,null)',\n", '')
v835.write_text(text, encoding='utf-8')

for name in [
    'patch_native_v0835_fast_overview_rain_taps.py',
    'patch_native_v0835_compile_helpers.py',
    'patch_native_v0836_video_blue_rivers_combined_detail.py',
    'patch_native_v0837_five_minute_freshness.py',
]:
    print(f'==> {name}', flush=True)
    subprocess.run([sys.executable, str(root / name)], check=True)

print('FloodSafe patches through v0.8.37 applied successfully')
