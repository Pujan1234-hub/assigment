...
(async()=>{
  ...
  try {
    ...
    await page.waitForFunction(()=>window.FloodSafeMobileMap&&document.getElementById('fsOpenMapBtn'),{timeout:15000});
    const isClosed=()=>page.evaluate(()=>!window.FloodSafeMobileMap.isOpen&&document.querySelector('.mapCanvas').getBoundingClientRect().height===0&&document.getElementById('fsOpenMapBtn').getAttribute('aria-expanded')==='false');
    assert.equal(await isClosed(),true,'map starts closed before any click');
    await page.click('.bottom a[href="#map"]');
    assert.equal(await isClosed(),true,'navigation scrolls to the map card without opening it');
    await page.$eval('#fsOpenMapBtn',e=>e.scrollIntoView({block:'center',behavior:'instant'}));
    await page.locator('#fsOpenMapBtn').click();
    await page.waitForFunction(()=>window.FloodSafeMobileMap.isOpen&&document.querySelector('.mapCanvas').getBoundingClientRect().height>0);
    ...
    await page.$eval('#fsOpenMapBtn',e=>e.scrollIntoView({block:'center',behavior:'instant'}));
    await page.evaluate(()=>document.addEventListener('click',e=>{window.__lastTestClick={id:e.target.id,text:e.target.textContent?.slice(0,60)}},true));
    await page.locator('#fsOpenMapBtn').click();
    console.log('TOGGLE CLICK',await page.evaluate(()=>({click:window.__lastTestClick,open:window.FloodSafeMobileMap.isOpen,label:document.getElementById('fsOpenMapBtn').textContent})));
    assert.equal(await page.evaluate(()=>window.FloodSafeMobileMap.isOpen),false);
    await page.evaluate(()=>{window.dispatchEvent(new Event('fslanguage'));window.dispatchEvent(new Event('fsriverheartbeat'));window.dispatchEvent(new Event('resize'))});
    assert.equal(await isClosed(),true,'language, refresh and resize cannot reopen a closed map');
    await page.$eval('#fsOpenMapBtn',e=>e.scrollIntoView({block:'center',behavior:'instant'}));
    await page.locator('#fsOpenMapBtn').click();
    ...
    await page.waitForFunction(()=>window.FloodSafeRain&&document.getElementById('rainAlertBtn'));
    assert.equal(await page.$eval('#rainAlertTestBtn',e=>e.disabled),true,'no push test before user consent');
    const notice=await page.evaluate(()=>{
      navigator.geolocation.clearWatch=()=>{};
      const gps=(latitude,longitude)=>{navigator.geolocation.watchPosition=ok=>{ok({timestamp:Date.now(),coords:{latitude,longitude,accuracy:10}});return 1};window.FloodSafeCurrentLocation.locate();return !document.getElementById('outsideNotice').hidden};
      const london=gps(51.5074,-0.1278),kathmandu=gps(27.7172,85.324);
      const source=window.FloodSafeMap?.map?.getSource('fs-user-location');
      if(window.FloodSafeMap?.map&&!source)throw Error('Current GPS marker source missing');
      const nepalDisplay=getComputedStyle(document.getElementById('outsideNotice')).display;
      navigator.geolocation.watchPosition=(_ok,fail)=>{fail({code:1,message:'test permission denied'});return 2};window.FloodSafeCurrentLocation.locate();
    ...
    assert.equal(await page.evaluate(()=>window.FloodSafeRain.state.point.kind),'gps');
    ...
    await page.bringToFront();await page.reload({waitUntil:'domcontentloaded',timeout:60000});
    await page.waitForFunction(()=>window.FloodSafeMobileMap&&document.getElementById('fsOpenMapBtn'));
    assert.equal(await isClosed(),true,'refresh resets even a previously opened map to closed');
    console.log('PASS map closed on initial load/reload; navigation and feed/language/resize events cannot open it; explicit button works');
