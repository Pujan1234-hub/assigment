const CACHE='floodsafe-nepal-v24-shell-10';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon.svg','./v23-hotfix.js','./v23-lang-police.js','./v23-river-map-v3.js','./v24-river-display.js','./v24-my-area-weather.js','./v24-safety-guard.js','./v24-upgrade.js'];
...
self.addEventListener('fetch',e=>{
  ...
  if(/\/v2[34]\/v23-upgrade\.js$/.test(u.pathname)){
    ...
    ]).then(async([base,hot,guard,live,riverDisplay,myWeather])=>{
      if(!base.ok)throw Error('base '+base.status);
      const a=await base.text(),b=hot.ok?await hot.text():'',c=guard.ok?await guard.text():'',d=live.ok?await live.text():'',e=riverDisplay.ok?await riverDisplay.text():'',f=myWeather.ok?await myWeather.text():'';
      return new Response(a+'\n;'+b+'\n;'+c+'\n;'+d+'\n;'+e+'\n;'+f,{status:200,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}});
