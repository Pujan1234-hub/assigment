...
PEOPLE_OUT = Path("data/floodsafe-people-status.json")
...
def clean_text(raw:str)->str:
    ...
    s=s.replace("पश्‍चिम","पश्चिम").replace("\u200d","").replace("\u200c","")
    return re.sub(r"\s+"," ",s).strip()

def np_num(s:str)->int:
    s=re.sub(r"[^०-९0-9सयहजार]+"," ",s).strip().translate(DIGITS)
    if not s: raise ValueError("empty Nepali number")
    toks=s.split()
    if "हजार" in toks:
        i=toks.index("हजार"); left=int(toks[i-1]) if i else 1
        rest=np_num(" ".join(toks[i+1:])) if toks[i+1:] else 0
        return left*1000+rest
    if "सय" in toks:
        i=toks.index("सय"); left=int(toks[i-1]) if i else 1
        rest=int("".join(t for t in toks[i+1:] if t.isdigit()) or "0")
        return left*100+rest
    nums=[t for t in toks if t.isdigit()]
    if not nums: raise ValueError(f"cannot parse Nepali number: {s!r}")
    return int("".join(nums))

def latest_bulletin_url(home_html:str)->str:
    p=LinkParser(); p.feed(home_html); candidates=[]
    for href,text in p.links:
        if "रसुवा बाढी" not in text or "मृत्यु" not in text: continue
    ...
    if not candidates: raise RuntimeError("No Rasuwa flood fatality bulletin found on Nepal Police homepage")
    return max(candidates)[1]

def parse_bulletin(url:str,raw:str)->dict:
    text=clean_text(raw)
    if "रसुवा बाढी" not in text or "जनाको मृत्यु" not in text: raise RuntimeError("Candidate page is not a Rasuwa flood fatality bulletin")
    body_m=re.search(r"जसमध्ये\s+(.*?)\s+गरी\s+[०-९0-9\sसयहजार]+?\s+जनाको शव फेला परेको छ",text)
    if not body_m: raise RuntimeError("Could not isolate current bulletin district paragraph")
    district_text=body_m.group(1); prefix=text[:body_m.start()]
    ...
    if not (date_hits and time_hits and total_hits): raise RuntimeError("Could not parse bulletin date/time/total")
    bs=date_hits[-1].translate(DIGITS); hh=int(time_hits[-1][0].translate(DIGITS)); mm=int(time_hits[-1][1].translate(DIGITS)); total=np_num(total_hits[-1])
    districts=[]
    for en,ne,label in DISTRICTS:
        m=re.search(label+r"\s+([०-९0-9\sसयहजार]+?)(?=\s*(?:जना)?\s*(?:,|र\s+|$))",district_text)
        if not m: raise RuntimeError(f"Could not parse district count: {ne}; paragraph={district_text}")
        districts.append({"name":en,"name_ne":ne,"deaths":np_num(m.group(1))})
    district_sum=sum(x["deaths"] for x in districts)
    if district_sum!=total: raise RuntimeError(f"Safety validation failed: district sum {district_sum} != published total {total}; parsed={districts}")
    now_np=datetime.now(ZoneInfo("Asia/Kathmandu")); iso=now_np.replace(hour=hh,minute=mm,second=0,microsecond=0).isoformat()
    return {"source":"Nepal Police","source_url":url,"event":"Rasuwa flood","event_ne":"रसुवा बाढी","cause":"Flood","cause_ne":"बाढी","total_deaths":total,"official_update_bs":bs,"official_update_time":f"{hh:02d}:{mm:02d} NPT","official_update_iso":iso,"districts":districts,"status":"verified","note":"Automatically synced from the latest Nepal Police Rasuwa flood bulletin. Time is Nepal Time (NPT, UTC+05:45). Data is written only when every district count is parsed and the district sum exactly matches the published total."}
...
def sync_people(data:dict)->bool:
    ...
    for k,v in fields.items():
        if people.get(k)!=v: people[k]=v; changed=True
    if changed: PEOPLE_OUT.write_text(json.dumps(people,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    return changed
...
def main()->int:
    ...
    else: print(f"No newer verified bulletin: {data['total_deaths']} at {data['official_update_time']}")
    people_changed=sync_people(data)
    if people_changed: print("Propagated latest Nepal Police total into Human Status snapshot")
    return 0

if __name__=="__main__":
    try: raise SystemExit(main())
    except Exception as e:
        print(f"FloodSafe police sync failed safely: {e}",file=sys.stderr); raise
