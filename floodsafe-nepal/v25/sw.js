const CACHE='floodsafe-nepal-v25-shell-33';
const SHELL=['./','./index.html','./map.html','./alerts.html','./people.html','./weather.html','./news.html','./sources.html','./shell.css','./live-core.css','./language.js','./resilience.js','./live-core.js','./national-news.js','./disaster-monitor.js','./today-monitor-v3.js','./human-current-guard.js','./district-road-live.js','./road-current-guard.js','./realtime-1s.js','./weather-live.js','./manifest.webmanifest','../v24/icon.svg'];
...
async function staticAsset(request){const c=await caches.open(CACHE),hit=await c.match(request,{ignoreSearch:true});const net=fetchWithTimeout(request,3500).then(async r=>{if(r&&r.ok){const u=new URL(request.url),key=new Request(u.origin+u.pathname);await c.put(key,r.clone())}return r}).catch(()=>null);if(hit){net.catch(()=>{});return hit}const r=await net;if(r)return r;throw Error('static unavailable')}
