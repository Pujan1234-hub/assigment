...
# The style code existed in v0.8.33, but the actual fs-rivers source could remain empty:
# v0.8.31 creates it empty before the async bundled waterways finish loading, and the later
# install path did not force the ready geometry back into the existing source.
# Alerts / notifications / 2 km safety are deliberately untouched.

# 1) Make the bundled national overview Nepal-safe before it becomes the fallback network.
old='''                    rivers.clear();\n                    rivers.addAll(all);\n                    overviewRivers.clear();\n                    overviewRivers.addAll(all);\n                    riversGeoJson = makeRiversGeoJson(all);'''
new='''                    List<RiverWay> safeAll=all;\n                    try{\n                        if(districtGeoJson!=null){\n                            JSONObject nepalRoot=new JSONObject(districtGeoJson);\n                            List<RiverWay> clipped=clipWaysToNepalDense(all,nepalRoot);\n                            if(clipped!=null&&!clipped.isEmpty())safeAll=clipped;\n                        }\n                    }catch(Exception ignoredClip){}\n                    rivers.clear();\n                    rivers.addAll(safeAll);\n                    overviewRivers.clear();\n                    overviewRivers.addAll(safeAll);\n                    riversGeoJson = makeRiversGeoJson(safeAll);'''
if old in m:
    m=m.replace(old,new,1)
elif 'List<RiverWay> safeAll=all;' not in m:
    raise SystemExit('v0.8.34 bundled geometry publish anchor missing')

# 2) Critical source-publication fix after async geometry loading.
post_variants=[
    'main.post(() -> { installGeoLayers(); lastRiverTileKey=""; refreshVisibleRiverTiles(); });',
    'main.post(()->{ installGeoLayers(); lastRiverTileKey=""; refreshVisibleRiverTiles(); });',
...
if 'if(riversGeoJson!=null&&!riversGeoJson.isEmpty())setGeo("fs-rivers",riversGeoJson);' not in m:
    replaced=False
    for p in post_variants:
        if p in m:
            m=m.replace(p,new_post,1);replaced=True;break
    if not replaced: raise SystemExit('v0.8.34 async source republish anchor missing')

# 3) If viewport loading is temporarily empty, never erase the safe national network.
...
# 4) Thin, clear cyan base + soft blue halo across all actual Nepal waterways.
...
# 5) Visible breathing flow on every river. CURRENT same-river official status overlays
# change core + halo to yellow/orange/red for alert/warning/danger.
...
m=m.replace('ensureRiverStatusLayer("fs-river-normal-status","fs-river-normal-status-layer","#24C7FF");',
            'ensureRiverStatusLayer("fs-river-normal-status","fs-river-normal-status-layer","#22C8FF");',1)
