(()=>{'use strict';
if(window.__fsGaugeBridgeV6)return;window.__fsGaugeBridgeV6=true;
const FRESH=24*60*60*1000,LOCAL_CONTEXT_KM=20;
...
function candidate(o,la,lo){const a=lat(o),b=lon(o);if(a===null||b===null)return null;const s=stage(o),isFresh=fresh(o),hasReading=s.level!==null||!!String(flat(o)?.status||'').trim()||!!stamp(o);if(!hasReading)return null;return{o,name:name(o),basin:basin(o),lat:a,lon:b,distanceKm:km(la,lo,a,b),time:stamp(o),fresh:isFresh,status:isFresh?s.status:'stale',currentStatus:s.status,statusLabel:statusLabel(isFresh?s.status:'stale'),level:s.level,warning:s.warning,danger:s.danger}}
...
window.FloodSafeGauge={forWaterway,nearby,nameMatch,statusLabel,LOCAL_CONTEXT_KM,scoreName,variants};
})();