...
# Prefer trimmed feeds so the app does not pick old history rows; raw history remains fallback only.
activity = activity.replace(
    'JSONObject official=getJson(BIPAD+"river/?limit=1000&_nativefull="+now);',
    'JSONObject official=getJson(BIPAD+"river-trimed/?limit=1000&_nativefull="+now);',
    1)
activity = activity.replace(
...
if old_rain not in activity:
    raise SystemExit('v0.8.9 rain feed anchor missing')
activity = activity.replace(old_rain, new_rain, 1)

# BIPAD/DHM payloads have changed field names over time; accept all observed time aliases.
activity = activity.replace(
    ...
    '"waterLevelOn","water_level_on","riverLevelOn","river_level_on","measuredOn","measured_on","measurementTime","measurement_time","observationTime","observation_time","observedAt","observed_at","datetime","dateTime","date_time","timestamp","_measurementTime","time"',
    1)
activity = activity.replace(
    ...
    '"rainfallOn","rainfall_on","measuredOn","measured_on","measurementTime","measurement_time","observationTime","observation_time","observedAt","observed_at","datetime","dateTime","date_time","timestamp","createdOn","updatedOn","date","time"',
    1)

# Preserve the exact official status text alongside the normalized FloodSafe safety stage.
old_return = 'return new RiverStation(name,district,a,o,level,warning,danger,at,fresh,stage,rank);'
new_return = 'return new RiverStation(name,district,a,o,level,warning,danger,at,fresh,stage,rank,raw);'
if old_return not in activity:
    raise SystemExit('RiverStation constructor call anchor missing')
activity = activity.replace(old_return, new_return, 1)
...
if old_cls not in activity:
    raise SystemExit('RiverStation class anchor missing')
activity = activity.replace(old_cls, new_cls, 1)

# ---- Native map: realtime status colours on whole matching river routes ----
# Unknown/stale geometry is neutral grey. Fresh direct gauges paint matching route overlays.
helper = helper.replace(
    'lineColor("#22e7ff"), lineWidth(2.45f), lineOpacity(1.0f)',
    'lineColor("#7f939c"), lineWidth(2.45f), lineOpacity(0.92f)',
    1)

# Make movement unmistakable on a phone: larger bright moving dots and stronger pulse trace.
helper = helper.replace(
    'ensurePointSource("fs-flow-particles", "fs-flow-particles-layer", "#d6fbff", 2.7f, 0.90f);',
    'ensurePointSource("fs-flow-particles", "fs-flow-particles-layer", "#f4feff", 4.0f, 1.0f);',
    1)
helper = helper.replace(
    'float glow=(float)(0.42+0.42*(0.5+0.5*Math.sin(ph*Math.PI*2.0)));flowTrace.setProperties(lineOpacity(glow),lineWidth(0.75f+0.45f*glow));',
    'float glow=(float)(0.48+0.50*(0.5+0.5*Math.sin(ph*Math.PI*2.0)));flowTrace.setProperties(lineOpacity(glow),lineWidth(1.05f+0.80f*glow));',
    1)

# Install four live-status line overlays after the static river layers.
status_anchor = '            ensurePointSource("fs-flow-particles", "fs-flow-particles-layer", "#f4feff", 4.0f, 1.0f);'
...
if 'fs-river-danger-status-layer' not in helper:
    if status_anchor not in helper:
        raise SystemExit('flow particle anchor missing')
    helper = helper.replace(status_anchor, status_install + status_anchor, 1)
...
if 'private void ensureRiverStatusLayer(' not in helper:
    if method_anchor not in helper:
        raise SystemExit('ensurePointSource anchor missing')
    helper = helper.replace(method_anchor, status_methods + method_anchor, 1)

# Recolour whenever station data refreshes.
old_refresh = '''        setGeo("fs-danger", stationGeo(snapshot, "danger"));
    }'''
new_refresh = '''        setGeo("fs-danger", stationGeo(snapshot, "danger"));
        refreshRiverStatusSources();
    }'''
if old_refresh not in helper:
    raise SystemExit('refreshStationSources anchor missing')
helper = helper.replace(old_refresh, new_refresh, 1)

# Recolour after overview/local river tile swaps too.
...
if 'versionCode 30' not in gradle or "versionName '0.8.10'" not in gradle:
    raise SystemExit('v0.8.10 version bump failed')
...
for marker in ['fs-river-normal-status-layer','fs-river-alert-status-layer','fs-river-warning-status-layer','fs-river-danger-status-layer','refreshRiverStatusSources','routeGaugeFor','lineColor("#7f939c")','"#f4feff", 4.0f, 1.0f','Official status:','Map colour:']:
