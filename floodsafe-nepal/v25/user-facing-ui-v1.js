...
function show(){open=true;renderButton();setTimeout(()=>{window.FloodSafeMap?.map?.resize?.();window.dispatchEvent?.(new CustomEvent('fsmapvisibility',{detail:{open:true}}));$('map')?.scrollIntoView?.({behavior:'smooth',block:'start'})},80)}
