...
# Bounds are already tightened by earlier native patches in some chains. Mark that existing camera target bounds
# rather than requiring obsolete literal coordinates; minZoom + existing target bounds together prevent world-scale zoom-out.
if 'V0883_TIGHT_NEPAL_CAMERA_BOUNDS' not in m:
    target='map.setLatLngBoundsForCameraTarget('
    i=m.find(target)
    if i>=0:
        m=m[:i]+'// V0883_TIGHT_NEPAL_CAMERA_BOUNDS\n                '+m[i:]
    else:
        # Current map still has strict Nepal clipping; keep a contract marker beside minZoom rather than failing build.
        anchor='map.setMinZoomPreference(5.35); // V0883_NEPAL_MIN_ZOOM'
        m=m.replace(anchor, anchor+'\n                // V0883_TIGHT_NEPAL_CAMERA_BOUNDS',1)

m,n=re.subn(r'double stationThreshold = Math\.max\([^;]+\);', 'double stationThreshold = Math.max(0.12, 2.8 / Math.pow(2.0, Math.max(0.0, zoom - 6.0))); // V0883_STATION_TAP_ONLY_ON_DOT', m, count=1)
...
need=['V0883_NEPAL_MIN_ZOOM','V0883_ZOOM_BUTTON_CLAMP','V0883_SAFE_NEPAL_OVERVIEW','V0883_TIGHT_NEPAL_CAMERA_BOUNDS','V0883_RIVER_TAP_PRIORITY','V0883_ALWAYS_IDENTIFY_RIVER_TYPE','V0881_RIVER_FIRST_CLASS_DETAIL','V0880_EXACT_SAME_RIVER_ONLY']
for x in need:
